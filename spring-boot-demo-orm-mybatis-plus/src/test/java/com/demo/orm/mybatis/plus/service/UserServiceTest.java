package com.demo.orm.mybatis.plus.service;
...
import com.demo.orm.mybatis.plus.SpringBootDemoOrmMybatisPlusApplicationTests;
...
public class UserServiceTest extends SpringBootDemoOrmMybatisPlusApplicationTests {
    ...
    public void testSave() {
        ...
        User testSave3 = User.builder().name("testSave3").password(SecureUtil.md5("123456" + salt)).salt(salt).email("test123@qq.com").phoneNumber("13500000001").status(1).lastLoginTime(new DateTime()).build();
    ...
    public void testSaveList() {
        ...
        for (int i = 4; i < 14; i++) {
            String salt = IdUtil.fastSimpleUUID();
            User user = User.builder().name("testSave" + i).password(SecureUtil.md5("123456" + salt)).salt(salt).email("testSave" + i + "@qq.com").phoneNumber(13500000000L + i + "").status(1).lastLoginTime(new DateTime()).build();
            userList.add(user);
        }
    ...
    public void testQueryByCondition() {
        ...
        wrapper.like("name", "Save1").or().eq("phone_number", "13500000001").orderByDesc("id");
