package com.demo.orm.jdbctemplate.dao.base;
...
import com.demo.orm.jdbctemplate.annotation.Ignore;
import com.demo.orm.jdbctemplate.annotation.Pk;
import com.demo.orm.jdbctemplate.annotation.Table;
import com.demo.orm.jdbctemplate.constant.Const;
...
public class BaseDao<T, P> {
    ...
    protected Integer insert(T t, Boolean ignoreNull) {
        ...
        String sql = StrUtil.format("INSERT INTO {table} ({columns}) VALUES ({params})", Dict.create().set("table", table).set("columns", columns)
                .set("params", params));
    ...
    public T findOneById(P pk) {
        ...
        log.debug("【执行SQL】SQL：{}", sql);
        log.debug("【执行SQL】SQL：{}", JSONUtil.toJsonStr(pk));
