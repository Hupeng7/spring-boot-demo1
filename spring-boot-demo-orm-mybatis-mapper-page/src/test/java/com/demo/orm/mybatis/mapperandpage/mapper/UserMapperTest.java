...
import com.demo.orm.mybatis.mapperandpage.SpringBootDemoOrmMybatisMapperPageApplicationTests;
...
public class UserMapperTest extends SpringBootDemoOrmMybatisMapperPageApplicationTests {
    ...
    public void testInsertList() {
        ...
        for (int i = 4; i < 14; i++) {
            String salt = IdUtil.fastSimpleUUID();
            User user = User.builder().name("testSave" + i).password(SecureUtil.md5("123456" + salt)).salt(salt).email("testSave" + i + "@qq.com").phoneNumber("1350000000" + i).status(1).lastLoginTime(new DateTime()).createTime(new DateTime()).lastUpdateTime(new DateTime()).build();
            userList.add(user);
        }
    ...
    public void testQueryByCondition() {
        ...
        // 过滤
        example.createCriteria().andLike("name", "%Save1%").orEqualTo("phoneNumber", "13500000001");
        // 排序
