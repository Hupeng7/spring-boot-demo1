...
public class UserDaoTest extends SpringBootDemoOrmJpaApplicationTests {
    ...
    public void testSave() {
        ...
        User testSave3 = User.builder().name("testSave3").password(SecureUtil.md5("123456" + salt)).salt(salt).email("testSave3@qq.com").phoneNumber("13500000001").status(1).lastLoginTime(new DateTime()).build();
    ...
    private void initData() {
        ...
        for (int i = 0; i < 10; i++) {
            String salt = IdUtil.fastSimpleUUID();
            int index = 3 + i;
            User user = User.builder().name("testSave" + index).password(SecureUtil.md5("123456" + salt)).salt(salt).email("testSave" + index + "@qq.com").phoneNumber("135000000" + index).status(1).lastLoginTime(new DateTime()).build();
            userList.add(user);
        }
