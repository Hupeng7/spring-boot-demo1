package com.demo.websocket.socketio.handler;
...
import com.demo.websocket.socketio.config.DbTemplate;
import com.demo.websocket.socketio.config.Event;
import com.demo.websocket.socketio.payload.BroadcastMessageRequest;
import com.demo.websocket.socketio.payload.GroupMessageRequest;
import com.demo.websocket.socketio.payload.JoinRequest;
import com.demo.websocket.socketio.payload.SingleMessageRequest;
...
public class MessageEventHandler {
    ...
    public void onDisConnect(SocketIOClient client) {
        if (client != null) {
            String token = client.getHandshakeData().getSingleUrlParam("tolen");
            // 模拟用户id 和token一致
            String userId = client.getHandshakeData().getSingleUrlParam("token");
            UUID sessionId = client.getSessionId();
     ...
     * @param message
     */
    private void sendToSingle(UUID sessionId, SingleMessageRequest message) {
    ...
    public void onGroupEvent(SocketIOClient client, AckRequest request, GroupMessageRequest data) {
        ...
        if (inGroup) {
            log.info("群号 {} 收到来自 {} 的群聊信息：{}", data.getGroupId(), data.getFromUid(), data.getMessage());
            sendToGroup(data);
        } else {
     ...
     * @param message
     */
    private void sendToGroup(GroupMessageRequest message) {
    ...
    public void sendToBroadcast(BroadcastMessageRequest message) {
        ...
        for (UUID clientId : dbTemplate.findAll()) {
            ...
            server.getClient(clientId).sendEvent(Event.BRAODCAST, message);
        }
    }
